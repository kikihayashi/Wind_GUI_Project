function WindRoseData = Wind_Rose(AxisHandle, WindDirectionData, Data, WindDirectionBins, WindSpeedBins, CirclePositions, Title, varargin)
    ...
    if ~isnan(FreeWindSector)
        ...
        FreeWindSector = NorthRelatedAngleToMathAngle(FreeWindSector / 180*pi);
    ...
    % title
    if ~isempty(Title)
        text(0, 1.35, ['{\bf ' sprintf('%s', Title) '}'], 'FontWeight','bold','FontSize',20,'HorizontalAlignment', 'center', 'Parent', AH);%Parent
    end
    ...
    if IsLegendEnabled
        ...
        for WindSpeedBinIndex = 1 : WindSpeedBinCount
            ...
            if any(WindSpeedBinEdges - floor(WindSpeedBinEdges));
                Format = '%4.1f-%4.1f(m/s)';
            else
                Format = '%2.0f-%2.0f(m/s)';
            end
...
function BinEdges = CalculateBinEdges(BinCenters, WithInfiniteEdges)
    ...
    % - Test -
    %
    % BinEdges = CalculateBinEdges([1:2:10], false);
    % BinEdges = CalculateBinEdges([1:2:10], true);
...
function [BinIndices, NumberOfValues] = SortDataIntoBins(Data, Bins, IsBinEdges, WithInfiniteEdges)
    ...
    % - Test -
    %
    % [BinIndices, NumberOfValues] = SortDataIntoBins([1 10 5 7 35 55]', [1:2:10], false, true);
    % [BinIndices, NumberOfValues] = SortDataIntoBins([1 10 5 7 35 55]', [1:2:10], false, false);
